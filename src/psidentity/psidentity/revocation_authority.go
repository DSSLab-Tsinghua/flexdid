...
func (i *Psidentity) GenerateLongTermRevocationKey() (*ecdsa.PrivateKey, error) {
...
// GenerateLongTermRevocationKey generates a long term signing key that will be used for revocation
func (i *Psidentity) LongTermRevocationKeyFromBytes(raw []byte) (*ecdsa.PrivateKey, error) {
...
func (i *Psidentity) CreateCRI(key *ecdsa.PrivateKey, unrevokedHandles []*math.Zr, epoch int, alg psidentity.RevocationAlgorithm, rng io.Reader, t Translator) (*CredentialRevocationInformation, error) {
	return createCRI(key, unrevokedHandles, epoch, alg, rng, i.Curve, t)
}

func createCRI(key *ecdsa.PrivateKey, unrevokedHandles []*math.Zr, epoch int, alg psidentity.RevocationAlgorithm, rng io.Reader, curve *math.Curve, t Translator) (*CredentialRevocationInformation, error) {
	if key == nil || rng == nil {
	...
	if alg == psidentity.ALG_NO_REVOCATION {
...
func (i *Psidentity) VerifyEpochPK(pk *ecdsa.PublicKey, epochPK *amcl.ECP2, epochPkSig []byte, epoch int, alg psidentity.RevocationAlgorithm) error {
	return verifyEpochPK(pk, epochPK, epochPkSig, epoch, alg)
}

func verifyEpochPK(pk *ecdsa.PublicKey, epochPK *amcl.ECP2, epochPkSig []byte, epoch int, alg psidentity.RevocationAlgorithm) error {
